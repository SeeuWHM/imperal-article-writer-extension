...
[![Version](https://img.shields.io/badge/version-2.5.0-green)](https://github.com/SeeuWHM/imperal-article-writer-extension/releases)
...
| **Proactive alerts** | The skeleton fires a `ctx.notify()` the moment a generation job lands in `review` (tracked in our own durable storage, not the kernel's in-memory diff state — see docs/extension.md "Proactivity"), so Webbee can tell the user their article is ready without being asked |
| **Honest patch results** | `patch_article` returns `matched`/`replaced_count` — if the instruction's target text genuinely isn't in the article, it reports that instead of a false "Updated section" |
...
├── pyproject.toml
└── tests/
    ├── test_handlers.py  # project/article CRUD, generation, patch, open_project — MockContext, no network
    ...
    ├── test_skeleton.py  # skeleton refresh + change-alert logic + direct ctx.notify() path
...
**Skeleton:** `skeleton_refresh_article_writer_overview` (project/article counts by status, TTL 60s, degrades to zeros if the backend is unreachable; also fires a direct `ctx.notify()` against its own durably-persisted baseline) and `skeleton_alert_article_writer_overview` (the kernel's own diff-based alert, kept as a harmless second layer — see docs/extension.md "Proactivity" for why the direct path is the reliable one).
...
../.venv-ext/bin/python -m pytest tests/ -q   # 73 tests — MockContext + monkeypatched call_backend, no network
